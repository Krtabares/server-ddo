...
async def print_on_request(request):
    ...
    if  'authorization' in request.headers:
        ...
        if present > session['expired_at']:
            ...
            session['expired_at'] = (present + timedelta(minutes = 15))
            await db.session_token.replace_one({'access_token': access_token}, session)
            print("paso por renovar")
...
async def totales_pedido(db, idPedido):
    ...
    try:
        ...
        desc_volumen=c.var(str)
        otros_descuentos=c.var(str)
        desc_adicional=c.var(str)
        desc_dpp=c.var(str)
        sub_total=c.var(str)
        impuesto=c.var(str)
        total=c.var(str)
        totalGravado=c.var(str)
        totalExento =c.var(str)
